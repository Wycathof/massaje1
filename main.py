...
def start_message(message):
    bot.send_message(message.chat.id, 'Привет! Чтобы записаться на маникюр, выбери дату и время', reply_markup=create_calendar())


@bot.callback_query_handler(func=lambda call: call.data.startswith('select_date'))
def callback_date_selected(call):
    date = call.data.split('|')[1]
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'Выбрана дата {date}. Теперь выбери время', reply_markup=create_time_selection(date))


@bot.callback_query_handler(func=lambda call: call.data.startswith('back_to_date'))
def callback_back_to_date(call):
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='Привет! Чтобы записаться на маникюр, выбери дату и время',
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('back_to_time'))
def callback_back_to_time(call):
    date = call.data.split('|')[1]
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'Выбрана дата {date}. Теперь выбери время',
                          reply_markup=create_time_selection(date))
...
def create_time_selection(date):
    ...
    time_selection.add(back_button)
...
def notify_admin(user_id, date, time):
    message = bot.send_message(ADMIN_CHAT_ID, f'Новый клиент записался на маникюр: {user_id}, {date} {time}. Подтвердить запись?',
...
def create_confirmation_keyboard(user_id):
    ...
    confirmation_keyboard.add(confirm_button, cancel_button, back_button)
